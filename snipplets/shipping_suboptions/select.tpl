...
<div class="js-shipping-suboption {{suboptions.name}}">
    {% if suboptions.options %}
        ...
        <div data-toggle="#{{ modal_id_val }}" class="js-modal-open btn-link btn-link pt-1">
            {% include "snipplets/svg/map-marker-alt.tpl" with {svg_custom_class: "svg-inline--fa mr-1 align-top svg-icon-primary"} %}
        ...
        {% embed "snipplets/modal.tpl" with{modal_id: modal_id_val, modal_class: 'bottom modal-centered-small js-modal-shipping-suboptions', modal_position: 'center', modal_transition: 'slide', modal_header: true, modal_footer: false, modal_width: 'centered', modal_zindex_top: true} %}
            ...
            {% endblock %}
            {% block modal_body %}
                <ul class="list-unstyled py-2 mt-3">
                    {% for option in suboptions.options %}
                        <li class="text-capitalize mb-3">{% include "snipplets/svg/map-marker-alt.tpl" with {svg_custom_class: "svg-inline--fa svg-icon-primary mr-2 d-flex float-left"} %} <span class="d-flex">{{ option.name | lower }}</span></li>
                    {% endfor %}
                </ul>
                <div class="mt-4"><span>{{ 'Cercanos al CP:'}}</span> <span class="text-primary font-weight-bold">{{cart.shipping_zipcode}}</span></div>
